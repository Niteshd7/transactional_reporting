view: pdt_user_activity {
  derived_table: {
    sql: SELECT  * FROM (         SELECT
               CONCAT('<a href="index.php?r=2&f=', IF('Activity' = 'Activity', MD5(IFNULL(ohg.name, oht.name)), oh.user), '">', FORMAT(COUNT(1), 0), '</a>') AS cnt_link,
               oh.user,
               a.admin_fullname,
...
LEFT OUTER JOIN
       ...
       GROUP BY
               IF('Activity' = 'Activity', activity, user)
      UNION ALL
         SELECT
               CONCAT('<a href="index.php?r=2&f=', IF('Activity' = 'Activity', MD5(IFNULL(ohg.name, oht.name)), oh.user), '">', COUNT(1), '</a>') AS cnt_link,
               oh.user,
               a.admin_fullname,
               IFNULL(ohg.name, oht.name)          AS activity,
               DATE_FORMAT(oh.t_stamp, '%m/%d/%Y') AS t_stamp,
...
LEFT OUTER JOIN
       ...
       GROUP BY
               IF('Activity' = 'Activity', activity, user)) a    ORDER BY admin_fullname ASC
 ;;
  }
